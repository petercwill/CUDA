...
__global__ void  heat_dist_kernel(float * in, float * out, unsigned int N);
...
int main(int argc, char * argv[])
{
  ...
  printf("%.5f\n", playground[index(17,37,N)]);
  printf("%.5f\n", playground[index(324,53,N)]);
  printf("%.5f\n", playground[index(52,652,N)]);
  // for(i = 0; i < N; i++)
  // {
  //   for(int j = 0; j <N; j++)
  //   {
  //     printf("%.2f, ", playground[index(i,j,N)]);
  //   }
  //   printf("\n");
  // }
  free(playground);
...
void  gpu_heat_dist(float * playground, unsigned int N, unsigned int iterations)
{
  ...
  // cuda allocation in global memory with error checking
  cudaError_t err=cudaMalloc((void **) &d_in, num_bytes);
  if( err !=cudaSuccess)
  {
  ...
  err=cudaMalloc((void **) &d_out, num_bytes);
  if( err !=cudaSuccess)
  {
   fprintf(stderr, "%s in %s at line %d\n",
    cudaGetErrorString(err), __FILE__, __LINE__);
  ...
  /* Copy initial array in temp */
  cudaMemcpy(d_in, playground, num_bytes, cudaMemcpyHostToDevice);
  for (int k = 0; k < iterations; k++)
  {
    heat_dist_kernel<<<dimGrid, dimBlock>>>(d_in, d_out, N);
    cudaMemcpy(d_in, d_out, num_bytes, cudaMemcpyDeviceToDevice);
  }
  cudaMemcpy(playground, d_out, num_bytes, cudaMemcpyDeviceToHost);


}

/**************** The Kernal Function called by the GPU verison**********/
__global__
void heat_dist_kernel(float * in, float * out, unsigned int N)
{
  int Col = blockIdx.x * blockDim.x + threadIdx.x;
  int Row = blockIdx.y * blockDim.y + threadIdx.y;
  int upper = N-1;
  float temp = 0;
  int gridpoint = index(Col, Row, N);

  // test to see if we're on the borders
  if ((Col == 0) || (Row == 0) || (Col == upper) || (Row == upper)){
    temp = in[gridpoint];
    out[gridpoint] = temp;
  }
  else if ((Col < upper) && (Row < upper)){

    temp = (
      in[index(Col, Row-1, N)]+
      in[index(Col, Row+1, N)]+
      in[index(Col-1, Row, N)]+
      in[index(Col+1, Row, N)]) / 4.0;
